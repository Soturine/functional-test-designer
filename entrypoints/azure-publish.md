...
- **Credentials:** runtime only — Azure CLI session, Microsoft Entra interactive sign-in (optional `azure-identity`) or a short-lived value in an environment variable the user names. They are never written to the plan, mapping, logs, diagnostics, package or report.
  - `--auth azure-cli` uses the user's existing Azure CLI session (`az login`). The `az` executable is located safely (no shell, no invented path) on every OS, including the `az.cmd` shim Windows installs. Azure DevOps-only accounts without an Azure subscription may need `az login --allow-no-subscriptions` first. A missing CLI reports `AZURE_CLI_UNAVAILABLE`; a CLI with no session reports `AZURE_CLI_NOT_AUTHENTICATED` — never a raw stack trace.
  - `--auth interactive` opens the browser **once** for interactive Microsoft Entra sign-in per publisher run; later reads reuse that same credential and its own token cache instead of prompting again. A cancelled or failed sign-in reports `INTERACTIVE_AUTH_FAILED`; a missing `azure-identity` package reports `CREDENTIAL_UNAVAILABLE`.
