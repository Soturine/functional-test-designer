...
Read [references/parallel-procedural-pipeline.md](references/parallel-procedural-pipeline.md) when multiple selected sources are present or the user specifies an inspection order. With no explicit order, analyze independent sources concurrently when safe and join at one evidence barrier. When the user naturally says first/then/last, or supplies an instruction file, normalize that intent into ordered source groups: files in one group may run concurrently, but the next group waits for the preceding barrier. Do not expose modes or require scheduler configuration. The instruction file controls orchestration only unless separately classified as product evidence. Order never expands scope and never changes authority.
...
Use this order: atomic CPs -> independent candidates -> independence review -> conservative merge/dedup -> final scenarios -> TCs. Record an internal objective reason for every merge; valid reasons include `SHARED_EXECUTION_OBSERVATIONS`, `INSEPARABLE_SAME_EVENT`, `SHARED_PASS_FAIL_BOUNDARY`, and `TRUE_SEMANTIC_DUPLICATE`. `SHARED_EXECUTION_OBSERVATIONS` requires equal complete execution signatures, one explicit execution family, same-execution observation, and no independent rerun. Never merge for `SAME_RF`, `SAME_SCREEN`, `SAME_ACTOR`, `SAME_SETUP`, `SAME_EVIDENCE_PACK`, `SAME_TITLE`, fewer tests, or performance optimization.
...
Read [references/procedural-execution.md](references/procedural-execution.md) before materializing Steps. Scenario design returns immutable Test identities first. Then use the structured orchestration in `scripts/procedural_pipeline.py` with the in-memory Evidence Packs; its workers may run independently after the freeze. Low-level synthesis remains in `scripts/procedural_execution.py`. Procedural synthesis may add execution fields but must pass the identity guard and must retain the normative oracle as the final expected result.
...
After every procedural result reaches the barrier, allow at most one additive reconciliation pass. Keep every existing TC and ID unchanged. A newly discovered independently rerunnable branch may append a TC only after the main authority confirms adequate oracle and lineage; implementation-only behavior stays a Question, Finding, or review item. Never remove, merge, renumber, or recursively redesign frozen cases.
