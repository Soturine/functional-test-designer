...
- Keep every authoritative relationship: a test whose scenario also involves other identifiers lists them in `related_identifiers`. With its claims' identifiers they become the test's `requirement_refs`/`source_identifiers` and show as chips on the collapsed HTML card (`TC-001 <title> [RN001] [RF002]`).
