...
Use `review_source_items`, `materialize_atomic_coverage`, and `audit_atomic_chain` from `scripts/source_coverage_audit.py` for the real Source Item -> Claim -> Clause -> CP boundary. Semantically equivalent claims may share one Clause only through an explicit equivalence key; retain all contributing provenance. Metrics must come from this independent inventory when it exists. Clause-plus-gap counts remain a presentation fallback for legacy outputs and are not valid proof that every source claim was identified.
