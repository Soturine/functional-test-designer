...
**Current validated run.** When a canonical run reaches `VALIDATED`, `finalize` atomically writes `<artifact-root>/.ftd/current-run.json` (`run_id`, `run_dir`, `canonical_digest`, `validated_at`). Failed, incomplete and chaos runs never write it. `/ftd-chaos`, `/ftd-azure`, `/ftd-check`, `/ftd-render` and `/ftd-azure-publish --prepare` resolve their run as: explicit `--run` (a run directory or a run id under the artifact root) > the current-run pointer > a clear error. Before use, the pointer is verified again (the run exists, is `VALIDATED`, its manifest is intact and its canonical suite matches the recorded digest); a stale or corrupt pointer is refused, never repaired by guessing. No run is ever selected by folder time or name. Post-generation actions started by `/ftd-gen` pass their own run directly.
