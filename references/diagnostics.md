...
Scenario-independence metrics describe the candidate-first review; they are not scores or count targets. In schema 2.2, `merge_candidates` is advisory metadata and `actual_merges` remains zero in the canonical atomic view. Legacy schema 1.2 retains its historical cohesion metrics for compatibility. `multi_cp_scenarios` is derived from final cases. A nonzero overcompression warning never rewrites output automatically.
