#!/usr/bin/env python3
"""Compact validation: the retained quality gates, honest gap metrics and the public
schema/cross-file validator for published suites (schema 2.2, compatible with 1.2)."""
...
GATES = (
    ...
    ("SOURCE_COVERAGE_VALID",
     "Every authoritative identifier is exercised by atomic Test Cases or carries an explicit disposition."),
    ("NORMATIVE_BASELINE_VALID",
     "Every testable claim has an atomic Acceptance Test Case with its oracle; additive stages left the frozen baseline unchanged."),
    ("ADDITIVE_EXPANSION_VALID",
     "Every expansion dimension, operator-error pattern, failure surface, test asset and use-case journey was evaluated with a valid disposition."),
    ("PROCEDURE_QUALITY_VALID",
     "Every Test Case has an executable procedure in the run locale; readiness derives only from material unknowns."),
    ("EVIDENCE_AND_REFERENCE_VALID",
     "Evidence references stay inside the selected scope and Questions/Findings link back to their Test Cases."),
    ("PIPELINE_INTEGRITY_VALID",
     "Stages ran in order through the official pipeline and every stage record matches the manifest hash chain."),
    ("PUBLICATION_VALID",
     "Public files validate against the schemas, render from canonical state only and are bound by publication digests."),
)
GATE_NAMES = tuple(name for name, _ in GATES)
# Published v2.2.x suites carry the former 28-gate set; they stay renderable.
LEGACY_GATE_NAMES = (
...
GENERIC_TITLES = {"header", "heading", "title", "cabecalho", "titulo", "section", "secao"}
FLOW_KINDS = {"USE_CASE", "ALTERNATIVE_FLOW", "EXCEPTION_FLOW"}


def build_gates(evidence: dict[str, str]) -> list[dict[str, str]]:
    """Gates are recorded only after the runtime has enforced them; a failure stops the run."""
    return [{"gate": name, "status": "PASS", "evidence": evidence.get(name) or why} for name, why in GATES]


def gap_metrics(
    ledger: list[dict[str, Any]], claims: list[dict[str, Any]], exercised_claims: set[str],
    challenge: list[dict[str, Any]], questions: list[dict[str, Any]], cases: list[dict[str, Any]],
) -> dict[str, int]:
    """Separate, honest gap dimensions; none of them hides behind claim-to-CP mapping."""
    return {
        "unmapped_extracted_claims": sum(
            claim["destination"] == "TEST" and claim["id"] not in exercised_claims for claim in claims
        ),
        "unaccounted_normative_units": sum(entry.get("disposition") is None for entry in ledger),
        "uncovered_use_case_flows": sum(
            entry["kind"] in FLOW_KINDS and not str(entry.get("disposition") or "").startswith("COVERED_")
            for entry in ledger
        ),
        "unresolved_business_test_assets": sum(item["disposition"] == "QUESTION_REQUIRED" for item in challenge),
        "unresolved_questions": len(questions),
        "blocked_normative_tests": sum(
            case.get("test_basis") == "ACCEPTANCE" and str(case.get("status", "")).startswith("BLOCKED")
            for case in cases
        ),
    }


def suite_metrics(index: dict[str, Any], cases: list[dict[str, Any]], questions: list[dict[str, Any]]) -> dict[str, Any]:
    """Diagnostics only: counts that describe a suite; none of them is a gate."""
    def count(values: Any) -> dict[str, int]:
        return dict(sorted(Counter(str(value) for value in values).items()))

    steps = [len(case.get("steps", [])) for case in cases]
    ledger = index.get("identifier_dispositions", [])
    by_kind: dict[str, dict[str, int]] = {}
    for entry in ledger:
        bucket = by_kind.setdefault(entry["kind"], {"total": 0, "covered": 0})
        bucket["total"] += 1
        bucket["covered"] += str(entry.get("disposition") or "").startswith("COVERED")
    priorities = Counter(case.get("priority") for case in cases)
    top = max(priorities.values(), default=0)
    return {
        "test_cases": len(cases),
        "by_basis": count(case.get("test_basis", "ACCEPTANCE") for case in cases),
        "by_dimension": count(case["expansion_dimension"] for case in cases if case.get("expansion_dimension")),
        "by_primary_type": count(case.get("primary_type", case.get("type")) for case in cases),
        "by_status": count(case["status"] for case in cases),
        "automation_suitability": count(case.get("automation_suitability", "-") for case in cases),
        "automation_readiness": count(case.get("automation_readiness", "-") for case in cases),
        "priority_distribution": count(case.get("priority") for case in cases),
        "priority_flattening_warning": len(cases) >= 5 and top / len(cases) > 0.9,
        "average_steps": round(sum(steps) / len(steps), 2) if steps else 0.0,
        "one_step_ratio": round(sum(value == 1 for value in steps) / len(steps), 3) if steps else 0.0,
        "identifier_coverage_by_kind": by_kind,
        "identifier_dispositions": count(entry.get("disposition") for entry in ledger),
        "findings": len(index.get("findings", [])), "questions": len(questions),
        "merge_candidates": len(index.get("merge_candidates", [])),
        "scenario_families": len(index.get("scenarios", [])),
        "gap_metrics": index.get("gap_metrics", {}),
        "gap_summary": gap_summary(index.get("gap_metrics", {})) if index.get("gap_metrics") is not None else None,
        "baseline_comparison": (index.get("baseline_comparison") or {}).get("status", "NOT_APPLIED"),
    }


def gap_summary(metrics: dict[str, int]) -> str:
    """Only an all-zero gap vector may be summarized as zero gaps."""
    open_items = {key: value for key, value in metrics.items() if value}
    if not open_items:
        return "0 gaps"
    return "; ".join(f"{key.replace('_', ' ')}: {value}" for key, value in open_items.items())
...
def validate(output_dir: Path) -> list[str]:
    ...
    for requirement in requirements:
        if isinstance(requirement, dict):
            ...
            title = str(requirement.get("source_title") or "")
            if title and " ".join(title.casefold().split()) in GENERIC_TITLES:
                errors.append(f"{requirement.get('id')} source_title {title!r} is not an official title")
    ...
    if schema_version == "2.2":
        ...
        if set(observed_gates) not in (set(GATE_NAMES), set(LEGACY_GATE_NAMES)) or len(
            observed_gates
        ) != len(set(observed_gates)):
...
def main() -> int:
    ...
    parser.add_argument(
        "--manifest", type=Path,
        help="also verify the run manifest, canonical digest and publication digests",
    )
    args = parser.parse_args()
    errors = validate(args.output.resolve())
    if args.manifest:
        from pipeline import IntegrityError, verify_manifest
        try:
