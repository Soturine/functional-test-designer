...
def synthesize_test_case(
    ...
    for index, item in enumerate(actions, 1):
        if item.get("independent_variant"):
            raise ValueError(
                f"{identity.id} action {index} is an independent variant; return it to scenario design"
            )
    ...
    case = {
        ...
        "type": "FUNCTIONAL",
        "objective": evidence_pack.get("objective", identity.title),
        ...
        "preconditions": deepcopy(evidence_pack.get("preconditions", [])),
